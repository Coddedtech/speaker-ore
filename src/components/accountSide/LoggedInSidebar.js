...
function LoggedInSidebar({ setisExclusive }) {
  ...
  if( ! JSON.parse(localStorage.getItem("@token") )?.userdata ) {
    ...
    loginButtons.push( <Button key="0" href="/login" variant="contained" style={{ 
      background: '#FFBF19', 
      width: '100%', 
      marginTop: 15
    }}>Sign In</Button> );

    loginButtons.push( <Button key="1" href="/login" variant="outlined" style={{ 
      color: '#FFBF19', 
      borderColor: '#FFBF19', 
      width: '100%',
      marginTop: 15
    ...
    loginButtons.push( <p style={{
      fontSize: 10,
      textAlign: 'center',
      fontWeight: 100,
      marginTop: 5
    }}>Single click Google Sign in.</p>);

    loginButtons.push( <h2 style={{
      marginTop: 50,
      fontSize: 12,
      textAlign: 'center',
      fontWeight: 700,
  ...
  for( const rKey in resoursesLinks ){
    resourceLinkHTML.push(<li><Button href={rKey} sx={{fontSize:10, fontWeight: 100, lineHeight: '14.06px', color: '#333'}}> { resoursesLinks[rKey] }</Button> </li>);
  }
    ...
    <div className="ep-right">
      <div className="eprcard">
        ...
        <p>
          ...
          {isLoggedin ? (
            <b style={{ fontWeight: "500", fontSize: 24, }}>{isLoggedin?.name}!</b>
          ) : (
            ""
          )}
          {!isLoggedin && <b style={{ fontWeight: "500", fontSize: 24, }}>Stranger</b>}
        </p>
        ...
        { !isLoggedin && <p style={{
          ...
          lineHeight: '14.06px',
          textAlign: 'center'
        }}>Are you an event manager or a speaker?</p> }
